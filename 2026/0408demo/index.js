...
const splitToSpans = (serifEl) => {
  const text = serifEl.textContent.trim();
  serifEl.innerHTML = text
    .split("")
    .map((ch) => `<span style="opacity:0">${ch}</span>`)
    .join("");
  return serifEl.querySelectorAll("span");
};
...
const buildScene = (index) => {
  ...
  // まず単語要素を組み立てる
  scene.innerHTML = sceneData.words
